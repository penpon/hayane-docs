...
Discordサーバにおけるユーザ同士の交流促進を、Discord DM機能とGoogleスプレッドシートを活用して自動化するシステムです。

## 📋 目次

1. [プロジェクト構成](#プロジェクト構成)
2. [主要機能](#主要機能)
3. [環境分離（開発・本番）](#環境分離開発本番)
4. [クイックスタート](#クイックスタート)
5. [テスト・CI/CD](#テストcicd)
6. [プロジェクト構造](#プロジェクト構造)
7. [関連ドキュメント](#関連ドキュメント)
...
Botは3つの専用Discordチャンネルに対して、リアルタイムでログを通知します。
...
**主要なセキュリティ機能**:
- **IPホワイトリスト**: JSON形式で管理、許可されたIPアドレスからのみAPIアクセスを受け付け
- **Bearer Token認証**: API Key による認証
- **レート制限**: Flask-Limiterによるエンドポイント別の制限
- **アクセスログ**: 成功・失敗の詳細記録
